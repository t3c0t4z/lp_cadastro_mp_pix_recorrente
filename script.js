...
const VALIDATION_PATTERNS = {
  instagram: /^[a-zA-Z0-9._]{1,30}$/, // SEM @ no regex
  whatsapp: /^\d{10,11}$/, // Só números, 10 ou 11 dígitos
...
function validateUsername(value, showFeedback = true) {
  ...
  // CORREÇÃO: Remove o @ antes de testar o regex
  const usernameWithoutAt = value.substring(1);

  if (!VALIDATION_PATTERNS.instagram.test(usernameWithoutAt)) {
    if (showFeedback) {
...
function validateWhatsApp(value, showFeedback = true) {
  ...
  // CORREÇÃO: Remove tudo exceto números antes de validar
  const numbersOnly = value.replace(/\D/g, "");

  if (!VALIDATION_PATTERNS.whatsapp.test(numbersOnly)) {
    if (showFeedback) {
...
themeToggle.addEventListener("click", () => {
  ...
  try {
    ...
    // n8n não deixa, então só ignoramos
  }
});
...
form.addEventListener("submit", async (e) => {
  ...
  try {
    ...
    if (data.init_point) {
      ...
      setTimeout(() => {
        console.log("➡️ Redirecionando para:", data.init_point);
      ...
      setTimeout(() => {
        ...
        document
          .querySelectorAll(".validation-feedback")
          .forEach((feedback) => {
